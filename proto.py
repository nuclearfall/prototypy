...
class Shape:
    property_spec = {
        ...
        "Type": {"type": "enum", "values": ["Rectangle", "Oval", "Triangle"]},
    }
...
class DrawingView(tk.Frame):
    ...
    def _build_ui(self):
        ...
        # Canvas (Drawing Area)
        self.canvas = tk.Canvas(self.pane, bg='white')
    ...
    def create_preview_shape(self, tool: str, start_x: int, start_y: int):
        """Creates a preview shape on the canvas (View logic)."""
        print("DrawingView.create_preview_shape: Creating preview shape.")
        # Store preview shape ID as View state
        ...
        self._preview_shape_id = preview_id # Store the ID
    ...
    def update_preview_shape(self, tool: str, start_x: int, start_y: int, current_x: int, current_y: int):
        """Updates the coordinates of the preview shape (View logic)."""
        if hasattr(self, '_preview_shape_id') and self._preview_shape_id and tool:
            if tool in ["rectangle", "oval"]: self.canvas.coords(self._preview_shape_id, start_x, start_y, current_x, current_y)
            elif tool == "triangle":
    ...
    def _update_properties_panel(self, selected_shape: Optional[Shape], refocus_info=None):
        ...
        if selected_shape:
            ...
            for prop_name in properties_to_display:
                ...
                elif prop_name == "Type": value = selected_shape.shape_type.capitalize()
